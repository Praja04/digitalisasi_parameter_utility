...
class RetailD9Controller extends Controller
{
    ...
    public function durasiStartMesinPerShiftRealtime(Request $request)
    {
        ...
        if ($filter === 'realtime') {
            ...
            $durasi = retail_d9::getStartMesinDurasiRealtime($tanggalStr);
    ...
    public function durasiStopMesinPerShiftRealtime(Request $request)
    {
        ...
        if ($filter === 'realtime') {
            ...
            $shifts = retail_d9::getShiftSchedule($carbonDate);
            $durasi = retail_d9::getOffMesinDurasiRealtime($tanggalStr);
    ...
    public function durasiOffMesinPerShift(Request $request)
    {
        ...
        if ($filter === 'realtime') {
            ...
            $hasil = retail_d9::getOffMesinDurasiPerShift($tanggal);
            ...
            while ($start->lte($end)) {
                ...
                $periode[$tanggal] = retail_d9::getOffMesinDurasiPerShift($tanggal);
                $start->addDay();
            }
    ...
    public function durasiStartMesinPerShift(Request $request)
    {
        ...
        if ($filter === 'realtime') {
            $tanggal = Carbon::now()->toDateString();
            $hasil = retail_d9::getStartMesinDurasiPerShift($tanggal);
        } elseif ($filter === 'tanggal' && $request->filled('tanggal')) {
            $tanggal = Carbon::parse($request->input('tanggal'))->toDateString();
            $hasil = retail_d9::getStartMesinDurasiPerShift($tanggal);
        } elseif ($filter === 'range' && $request->filled(['start_date', 'end_date'])) {
            ...
            while ($start->lte($end)) {
                $tanggal = $start->toDateString();
                $periode[$tanggal] = retail_d9::getStartMesinDurasiPerShift($tanggal);
                $start->addDay();
            }
