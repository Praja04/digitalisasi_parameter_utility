...
class RetailD8Controller extends Controller
{
    ...
    public function durasiStartMesinPerShiftRealtime(Request $request)
    {
        ...
        if ($filter === 'realtime') {
            ...
            $shifts = retail_d8::getShiftSchedule($carbonDate);
            $durasi = retail_d8::getStartMesinDurasiRealtime($tanggalStr);
    ...
    public function durasiStopMesinPerShiftRealtime(Request $request)
    {
        ...
        if ($filter === 'realtime') {
            ...
            $shifts = retail_d8::getShiftSchedule($carbonDate);
            $durasi = retail_d8::getOffMesinDurasiRealtime($tanggalStr);
    ...
    public function durasiOffMesinPerShift(Request $request)
    {
        ...
        if ($filter === 'realtime') {
            ...
            $hasil = retail_d8::getOffMesinDurasiPerShift($tanggal);
            ...
            while ($start->lte($end)) {
                ...
                $periode[$tanggal] = retail_d8::getOffMesinDurasiPerShift($tanggal);
                $start->addDay();
            }
    ...
    public function durasiStartMesinPerShift(Request $request)
    {
        ...
        if ($filter === 'realtime') {
            $tanggal = Carbon::now()->toDateString();
            $hasil = retail_d8::getStartMesinDurasiPerShift($tanggal);
        } elseif ($filter === 'tanggal' && $request->filled('tanggal')) {
            $tanggal = Carbon::parse($request->input('tanggal'))->toDateString();
            $hasil = retail_d8::getStartMesinDurasiPerShift($tanggal);
        } elseif ($filter === 'range' && $request->filled(['start_date', 'end_date'])) {
            ...
            while ($start->lte($end)) {
                $tanggal = $start->toDateString();
                $periode[$tanggal] = retail_d8::getStartMesinDurasiPerShift($tanggal);
                $start->addDay();
            }
