...
class SensorBoilerController extends Controller
{
    public function generate(Request $request)
    {
        ...
        while ($current < $end) {
            ...
            $row = DB::table('readsensors_boiler')
            ->where('waktu', '>=', $current)
                ->where('waktu', '<', $nextHour)
                ->orderBy('waktu', 'asc') // ambil paling awal di jam itu
                ->limit(1)
                ->first();
        ...
        return response()->json([
            'status' => true,
            'message' => 'Data boiler per jam (super cepat)',
            'data' => $data->sortByDesc('waktu')->values()
        ]);
    }
