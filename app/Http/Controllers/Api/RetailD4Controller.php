...
use App\Models\Retail\retail_d4_nozzle2;
use Illuminate\Http\Request;
use Carbon\Carbon;

class RetailD4Controller extends Controller
{
    //
    ...
    public function durasiStartMesinPerShiftRealtime(Request $request)
    {
        ...
        if ($filter === 'realtime') {
            ...
            $shifts = retail_d4::getShiftSchedule($carbonDate);
            $durasi = retail_d4::getStartMesinDurasiRealtime($tanggalStr);
    ...
    public function durasiStopMesinPerShiftRealtime(Request $request)
    {
        ...
        if ($filter === 'realtime') {
            ...
            $shifts = retail_d4::getShiftSchedule($carbonDate);
            $durasi = retail_d4::getOffMesinDurasiRealtime($tanggalStr);
    ...
    public function durasiOffMesinPerShift(Request $request)
    {
        ...
        if ($filter === 'realtime') {
            ...
            $hasil = retail_d4::getOffMesinDurasiPerShift($tanggal);
            ...
            while ($start->lte($end)) {
                ...
                $periode[$tanggal] = retail_d4::getOffMesinDurasiPerShift($tanggal);
                $start->addDay();
            }
    ...
    public function durasiStartMesinPerShift(Request $request)
    {
        ...
        if ($filter === 'realtime') {
            $tanggal = Carbon::now()->toDateString();
            $hasil = retail_d4::getStartMesinDurasiPerShift($tanggal);
        } elseif ($filter === 'tanggal' && $request->filled('tanggal')) {
            $tanggal = Carbon::parse($request->input('tanggal'))->toDateString();
            $hasil = retail_d4::getStartMesinDurasiPerShift($tanggal);
        } elseif ($filter === 'range' && $request->filled(['start_date', 'end_date'])) {
            ...
            while ($start->lte($end)) {
                $tanggal = $start->toDateString();
                $periode[$tanggal] = retail_d4::getStartMesinDurasiPerShift($tanggal);
                $start->addDay();
            }
