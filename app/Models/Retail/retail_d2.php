...
class retail_d2 extends Model
{
    //
    use HasFactory;

    protected $table = 'retail_d2';
    ...
    //             ) AS rn
    //         FROM all_targets t
    //         JOIN retail_d2 r ON r.ts <= t.target_ts
    ...
    //     // Ambil data dari tabel retail_d2_nozzle1
    //     $totalNozzleAktif = retail_d2_nozzle1::whereBetween('ts', [$start, $carbonDate])
    //         ->get()
    ...
    //     foreach ($shifts as $shift) {
    //         $totalNozzleAktif = retail_d2_nozzle1::whereBetween('ts', [$shift['start'], $shift['end']])
    //             ->get()
    //             ->reduce(function ($carry, $item) {
    //                 return $carry + ($item->nozzle_1 == 1 ? 1 : 0) + ($item->nozzle_2 == 1 ? 1 : 0);
    //             }, 0);
    ...
    public static function getPerformanceOutput($tanggal = null)
    {
        ...
        $awalCounter = DB::table('retail_d2')
            ->where('ts', '<=', $start)
            ->where('total_counter', '!=', 0)
            ->orderByDesc('ts')
            ->limit(1)
        ...
        $akhirCounter = DB::table('retail_d2')
            ->where('ts', '<=', $carbonDate)
            ->where('total_counter', '!=', 0)
            ->orderByDesc('ts')
            ->limit(1)
