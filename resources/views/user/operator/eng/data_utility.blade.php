    ...
    <script>
        $(document).ready(function() {
            ...
            $('.card-unit').on('click', function() {
                ...
                $('#exportChemexport-containericalBtn').toggle(currentUnit === 'Chemical');
            ...
            function buildListrikTable(entry) {
                ...
                const operatorRow =
                    `<tr><th>Operator</th>${headers.map(p => `<td>${entry.operator?.[p] ?? '-'}</td>`).join('')}</tr>`;
                const usageRow =
                    `<tr><th>Usage </th>${headers.map(p => `<td>${entry.usage?.[p] ?? '-'}</td>`).join('')}</tr>`;
            ...
            function buildChemicalTable(entry) {
                ...
                const rows = entry.data.map(d => {
                    const group = ['nilai_pemakaian', 'area', 'operator', 'notes', 'running_hour'].map(
                        attr => {
